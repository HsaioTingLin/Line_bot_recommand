...
# database connect
ip = "192.168.1.176"
db = DataBaseConnector(ip)
recommender = Recipe_recommender(ip)
...
def kafka():
    if request.values:
        ...
        ingredient_name = request.values.get("ingredient")
...
def process_postback_event(event):
    ...
    if pb_function == "menu":
        ...
        if action[0] == "manual":
            ...
            try:
                ssh_client.connect(hostname='192.168.196.172', port=22, username='pi', password='pi')
                line_bot_api.push_message(
                    user_id,
